 ...
 */
ContinuousPC2::ContinuousPC2(const OT::Sample &data,
...
ContinuousPC2::getSeparator(const gum::UndiGraph & /*g*/, gum::NodeId y,
 ...
 * cut)
 */
bool ContinuousPC2::testCondSetWithSize(gum::UndiGraph &g,
...
gum::UndiGraph ContinuousPC2::inferSkeleton()
{
  gum::UndiGraph g;
  tester_.clearCache();
  sepset_.clear();
...
std::vector<std::string> ContinuousPC2::namesFromData(void) const
{
...
NamedJunctionTree ContinuousPC2::learnJunctionTree()
{
  if (jt_done_)
    return jt_;
  if (!pdag_done_)
...
gum::MixedGraph ContinuousPC2::learnPDAG()
{
  if (pdag_done_)
    return pdag_;
  if (!skel_done_)
...
gum::UndiGraph ContinuousPC2::learnSkeleton()
{
  if (skel_done_)
    return skel_;
...
gum::JunctionTree
ContinuousPC2::deriveJunctionTree(const gum::UndiGraph &g) const
{
...
gum::DAG ContinuousPC2::deriveDAG(const gum::MixedGraph &p) const
{
...
std::string ContinuousPC2::skeletonToDot(const gum::UndiGraph &skeleton)
{
  std::stringstream ss;
...
std::string ContinuousPC2::PDAGtoDot(const gum::MixedGraph &pdag)
{
  std::stringstream ss;
...
double ContinuousPC2::getPValue(gum::NodeId x, gum::NodeId y) const
{
  gum::Edge e(x, y);
...
double ContinuousPC2::getTTest(gum::NodeId x, gum::NodeId y) const
{
  gum::Edge e(x, y);
  if (ttests_.exists(e))
  {
...
OT::Indices ContinuousPC2::getSepset(gum::NodeId x, gum::NodeId y) const
{
  gum::Edge e(x, y);
  if (pvalues_.exists(e))
  {
...
void ContinuousPC2::setVerbosity(bool verbose)
{
  verbose_ = verbose;
};

bool ContinuousPC2::getVerbosity() const
{
  return verbose_;
};

const std::vector<gum::Edge> &ContinuousPC2::getRemoved() const
{
  return removed_;
}

bool ContinuousPC2::isRemoved(gum::NodeId x, gum::NodeId y) const
{
  return (std::find(removed_.begin(), removed_.end(), gum::Edge(x, y)) !=
          removed_.end());
}

bool ContinuousPC2::isRemoved(const std::string &x, const std::string &y) const
{
  return isRemoved(idFromName(x), idFromName(y));
}

double ContinuousPC2::getPValue(const std::string &x,
                               const std::string &y) const
{
  return getPValue(idFromName(x), idFromName(y));
}
double ContinuousPC2::getTTest(const std::string &x,
                              const std::string &y) const
{
...
ContinuousPC2::getSepsetNames(const std::string &x, const std::string &y) const
{
...
gum::NodeId ContinuousPC2::idFromName(const std::string &n) const
{
  const auto &description = tester_.getDataDescription();
  for (OT::UnsignedInteger i = 0; i < description.getSize(); i++)
  {
...
std::vector<std::string> ContinuousPC2::getTrace() const
{
  const auto &description = tester_.getDataDescription();
